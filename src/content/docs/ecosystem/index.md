...
flowchart TB
  User([Researcher])
  CLI["nemar CLI"]
  Web["www.nemar.org<br/>dataset browser"]
...
| **Dataset browser** | `www.nemar.org` | The dataset browser (Astro) | external |
...
flowchart LR
  ...
  F --> G[Public on<br/>data.nemar.org + www.nemar.org]
...
5. On publication the dataset becomes public on the **data plane** (`data.nemar.org`) and is
   surfaced in the **browser** (`www.nemar.org`); recordings are converted for the **viewer**.
