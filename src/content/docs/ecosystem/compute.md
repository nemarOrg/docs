...
![NEMAR edge and compute diagram showing canonical BIDS data, Zarr conversion, edge delivery, browser analysis, and a planned NEMAR integration with Tapis through OneSciencePlace.](/figures/nemar-edge-compute.png)
...
dataset, version, and workflow seamlessly to those established compute resources so they can become
a reproducible analysis.

That partnership is what makes the data-to-compute loop practical. It is also why a planned NEMAR
integration with Tapis through [OneSciencePlace (OSP)](https://www.onescienceplace.org/) matters:
OSP uses Tapis for job and data lifecycle management, and its teams include UCLA, SDSC, and TACC.
The goal is to connect NEMAR's dataset and citation contracts to national HPC infrastructure rather
than sending researchers back to rebuild transfers and environments by hand.

## The planned HPC direction

We envision using Tapis through OneSciencePlace as an API layer for connecting NEMAR workflows to
national HPC infrastructure. In plain terms, a researcher or an agent would be able to describe a
reproducible job, select an approved compute resource, provide the dataset/version and inputs, and
receive tracked outputs without manually rebuilding the entire transfer and execution workflow.
